...
max_result = -float('inf') # 반대로 -로 설정 
min_result = float('inf')
def dfs(idx, current): 
    # 연산자 몇개 썼는지 알기 위해서 idx 설정 
    # current는 연산 결과값 저장 
    ...
    # 연산자가 4개 이기 때문에 4번 돌림 
    for i in range(4):
        if operators[i] > 0: # 연산자가 존재할 때까지 !! 계속 빼주니깐 
            operators[i] -= 1 # 존재하면 그 줄(그 연산자)에 -1 해주고 어떤 연산자인지 조건문에서 if elif 문으로 넘어감
            if i == 0: # 더하기이면 
